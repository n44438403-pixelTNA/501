...
const RevisionHubComponent: React.FC<Props> = ({ user, onTabChange, settings, onNavigateContent, onUpdateUser }) => {
    ...
    const handleDownloadHubData = () => {
        // Cost 10
        if (user.credits < 10) {
            setAlertConfig({isOpen: true, type: 'ERROR', message: "Insufficient Credits. Costs 10 Coins."});
            return;
        }
        ...
        <div className="space-y-6 pb-24 p-4 animate-in fade-in relative">
            ...
            {/* DAILY BRIEFING (Restricted in Free Mode) */}
             <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100 flex items-start gap-3 relative overflow-hidden z-10">
                ...
                <div className="z-10">
                    <p className="text-xs font-bold text-indigo-900 mb-1">Daily Routine & Briefing</p>
                    {/* DAILY ROUTINE DISPLAY */}
                    {user.dailyRoutine && user.dailyRoutine.date === todayStr && (
                        <div className="mb-3 bg-white/60 p-2 rounded-lg border border-indigo-100/50">
                            <p className="text-xs font-bold text-indigo-800 mb-1">🎯 Today's Focus: {user.dailyRoutine.focusArea}</p>
                            <div className="space-y-1">
                                {user.dailyRoutine.tasks.slice(0, 2).map((task, i) => (
